...
**唯一维护处**：[`cloudfunctions/common/merchantGate.js`](../cloudfunctions/common/merchantGate.js) 中的 `OWNER_OPENIDS`。

部署前执行 `npm run sync:cloud`，将 `merchantGate.js` 同步到各云函数 `./common/`。业务云函数（含 `login`、`staff`、`goods`、`order` 等）均通过 `require('./common/merchantGate')` 鉴权，**勿**在单个 `index.js` 再写一份白名单。
