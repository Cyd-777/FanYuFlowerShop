> ⚠️ **已迁移** → 见 [./商品模块.md](./商品模块.md) 及子文档。下文保留作对照，**勿再在此维护**。
...
**确认页填单**（地址、备注、核对商品、点击「提交订单」）及之后的创建订单、支付、订单列表/详情、商家处理、核销，归属 **[订单模块](./订单模块.md)**。
...
- 确认页填单、提交订单、支付、订单列表/详情 → [订单模块](./订单模块.md)
- 会员积分、地址簿长期管理（地址选择在确认页，属订单流）
- 花卉百科 Tab 内容 → [智库模块](./智库模块.md)（仅商品详情链到百科卡片）
