...
export function parsePendingStaffInvite(raw: unknown): { type: 'code' | 'token'; value: string } | null {
...
export async function acceptStaffInvite(token: string): Promise<{ role: StaffRole; roleLabel: string; name: string }> {
...
export async function acceptStaffInviteByCode(code: string): Promise<{ role: StaffRole; roleLabel: string; name: string }> {
  const normalized = normalizeStaffInviteCode(code)
  if (!isStaffInviteCode(normalized)) {
    throw new Error('请输入 6 位邀请码')
  }
